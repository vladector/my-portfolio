...
// --- КОМПОНЕНТ ОДНОГО ГЛИТЧ-СИМВОЛА ---
const GlitchChar = ({ char, color }: { char: string, color: string }) => {
        ...
        const glitchInterval = setInterval(() => {
            if (Math.random() > 0.7) {
                ...
                setStyle({ 
                  ...
                  fontWeight: weights[Math.floor(Math.random() * weights.length)] as any, 
                  fontStyle: styles[Math.floor(Math.random() * styles.length)], 
                  scale: Math.random() > 0.5 ? 1.2 : 0.9, 
    ...
    return ( 
      <motion.span 
        ...
        style={{ color: color, fontFamily: style.fontFamily, fontWeight: style.fontWeight, fontStyle: style.fontStyle, y: style.y }} 
...
// --- КОМПОНЕНТ ЗАГОЛОВКА С УМНЫМ ПЕРЕНОСОМ ---
const GlitchTitle = ({ text, color, subColor }: { text: string, color: string, subColor: string }) => {
    const words = text.split(' '); // Разделяем на "ВЛАД" и "КАТАЕВ"
    return (
        <div className="flex flex-col items-center justify-center leading-none select-none w-full">
            ...
            <div className="flex flex-col md:flex-row flex-wrap justify-center items-center w-full text-5xl sm:text-7xl md:text-8xl lg:text-[140px] font-black tracking-tighter uppercase leading-[0.8] md:leading-[0.85] mix-blend-difference text-center md:gap-x-8">
                {words.map((word, wordIndex) => (
                    <div key={wordIndex} className="whitespace-nowrap">
                        {word.split('').map((char, index) => (
                            <GlitchChar key={index} char={char} color={color} />
                        ))}
                    </div>
                ))}
            </div>
    ...
    <section id="hero" className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden transition-colors duration-1000">
      ...
      <div className="w-full max-w-[1200px] mx-auto text-center relative z-10 flex flex-col items-center">
        ...
        >
          ...
          <motion.p 
            className="text-base md:text-xl font-sans font-medium tracking-normal mb-8 md:mb-12 max-w-2xl mx-auto leading-relaxed text-center px-4" 
